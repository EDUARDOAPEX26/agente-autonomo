"""
AGENTE AUTÔNOMO — Railway Cloud Agent v4.3.1
Mudancas vs v4.3:
- Fix 409 GitHub: sleep com jitter (2 + tentativa*2 + random) em vez de sleep(1) fixo
- SYSTEM_PROMPT mais firme: sem markdown, sem invenção de dados/datas/servidores
- max_tokens reduzidos nas tarefas internas (60-100 em vez de 150-300)
"""
...
def _handle_shutdown(sig, frame):
    ...
    salvar_memoria_github(get_memoria_compartilhada())
...
def _worker_fila():
    while True:
        try:
            ...
            tarefa    = item["tarefa"]
            dados     = item["dados"]
            log("WORKER", f"Processando: {tarefa} (id={tid})")
            resultado = _processar_tarefa_interna(tarefa, dados)
            _resultados_tarefas[tid] = {
                "status": "ok", "tarefa": tarefa, "resultado": resultado,
                "ts": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
            }
            if len(_resultados_tarefas) > 50:
                for k in sorted(_resultados_tarefas.keys())[:-50]:
                    _resultados_tarefas.pop(k, None)
            log("WORKER", f"Concluído: {tarefa} (id={tid})")
        except Exception:
            pass

def _processar_tarefa_interna(tarefa: str, dados: dict) -> str:
    if tarefa == "registrar_licao":
        msg  = dados.get("mensagem_usuario", "")
        resp = dados.get("ultima_resposta", "")
        if msg:
            licao = _extrair_licao_via_llm(msg, resp)
            if licao:
                registrar_licao(licao["gatilho"], resp, licao)
                return f"licao_registrada: {licao.get('gatilho','')[:60]}"
        return "licao_nao_registrada"
    if tarefa == "buscar_licoes":
        licoes = buscar_licoes_relevantes(dados.get("pergunta", ""))
        return formatar_licoes_para_prompt(licoes) or "sem_licoes"
    if tarefa == "buscar_resumo":
        return buscar_resumo_relevante(dados.get("pergunta", "")) or "sem_resumo"
    ...
    if tarefa == "status_resumo":
        ...
        return json.dumps({
            ...
            "itens_por_bloco": {b: len(v) for b, v in resumo.get("blocos", {}).items()},
        }, ensure_ascii=False)
...
# ── FASTAPI ───────────────────────────────────────────────────────────────────
app = FastAPI()
estado = {"ciclo": 0, "objetivo": "iniciando", "cpu": 0.0, "mem": 0.0, "ultima_tarefa": "", "online": True}
...
def status():
    ...
    resumo   = _carregar_livro_local("resumo")
    return {
        "ciclo": estado["ciclo"], "objetivo": estado["objetivo"],
        "cpu": estado["cpu"], "mem": estado["mem"],
        "ultima_tarefa": estado["ultima_tarefa"],
        "tavily_uso": controle.get("tavily_uso", 0), "tavily_max": MAX_TAVILY,
        "online": True, "timestamp": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
...
def get_memoria_endpoint():
    try:
        ...
        for linha in linhas:
            ...
            except: entradas.append({"raw": linha.strip()})
        mem = get_memoria_compartilhada()
        return {"total_log": len(linhas), "ultimas": entradas,
...
def executar_tarefa_endpoint(payload: TarefaPayload):
    ...
    _fila_tarefas.put({"id": tid, "tarefa": payload.tarefa, "dados": payload.dados})
    log("TAREFA", f"Enfileirada: {payload.tarefa} (id={tid})")
    return {"status": "ok", "enfileirada": True, "tarefa": payload.tarefa, "id": tid}

@app.get("/resultado/{tarefa_id}")
def get_resultado(tarefa_id: str):
    return _resultados_tarefas.get(tarefa_id, {"status": "pendente", "id": tarefa_id})
...
def get_resumo_bloco(bloco: str):
    if bloco not in {"decisoes", "bugs", "correcoes", "limitacoes", "insights"}:
...
def buscar(query: str, assunto: str = "geral", top_n: int = 5):
    chave = f"{assunto}:{query}"
    agora = time.time()
    if chave in _cache_busca and agora - _cache_busca[chave]["ts"] < BUSCA_CACHE_TTL:
        return {"assunto": assunto, "query": query, "resultados": _cache_busca[chave]["resultado"], "cache": True}
    ...
    _cache_busca[chave] = {"resultado": resultados, "ts": agora}
...
def _buscar_tfidf(query: str, entradas: list, top_n: int = 5) -> list:
    ...
    if not indice: return entradas[:top_n]
    tf_q    = _calcular_tf(_tokenizar(query))
    vetor_q = {t: tf_q[t] * indice["idf"].get(t, 1.0) for t in tf_q}
    scores  = [((_similaridade_cosseno(vetor_q, v) + entradas[i].get("confianca", 0.7) * 0.05), i)
               for i, v in enumerate(indice["vetores"])]
    scores.sort(reverse=True)
    return [{"data": entradas[i].get("data",""), "pergunta": entradas[i].get("pergunta","")[:150],
             "resposta": entradas[i].get("resposta","")[:150], "confianca": entradas[i].get("confianca",0.7),
             "origem": entradas[i].get("origem","local"), "score": round(s, 4)}
    ...
    for tentativa in range(3):
        try:
            ...
            if r2.status_code in (200, 201):
                ...
                time.sleep(2 + tentativa * 2 + random.uniform(0, 1))  # FIX: jitter
            else:
...
def registrar_tarefa_na_memoria(tarefa: str, resultado: str):
    ...
    mem["aprendizados"].append({"data": datetime.now().strftime("%d/%m/%Y %H:%M"),
                                "origem": "nuvem", "tarefa": tarefa[:100], "resultado": resultado[:200]})
...
def sincronizar_github_se_necessario():
    ...
    if ciclos_desde_sync["n"] >= SYNC_A_CADA:
        ciclos_desde_sync["n"] = 0
        threading.Thread(target=salvar_memoria_github, args=(get_memoria_compartilhada(),), daemon=True).start()

# ── LLM ───────────────────────────────────────────────────────────────────────
_groq_chave_ativa = {"k": 1}

def chamar_llm(prompt: str, max_tokens: int = 200, system: str = "") -> str:
    ...
    if HUGGING_KEY:
        try:
            ...
            if isinstance(resultado, list) and resultado:
                ...
                if txt:
                    warn("LLM", "Usando HuggingFace como fallback de emergência")
                    return txt
        except Exception as e:
...
def pode_usar_tavily() -> bool:
    ...
    if dados["tavily_uso"] >= MAX_TAVILY: return False
    dados["tavily_uso"] += 1
...
PADROES_CORRECAO = [
    ...
    "já falei isso", "não faça isso", "isso é um erro", "não funciona assim",
    "correção:", "na verdade", "você entendeu errado", "isso não é correto",
    "você inventou", "está confundindo", "nunca faça", "não repita",
    "anti-padrão", "antipadrão", "evite isso",
]

PADROES_ENSINO = [
    "lembre que", "guarde isso", "regra:", "importante:",
    "salva isso", "para o futuro:", "a partir de agora",
...
def _extrair_licao_via_llm(msg_usuario: str, resposta_errada: str) -> dict | None:
    prompt = (
        ...
        f"Mensagem: \"{msg_usuario[:300]}\"\n"
        f"Resposta errada: \"{resposta_errada[:200]}\"\n\n"
        f"Retorne APENAS este JSON:\n"
        f"{{\"gatilho\":\"situação (máx 15 palavras)\","
        f"\"erro\":\"o que errou (máx 20 palavras)\","
        f"\"correto\":\"o que fazer (máx 20 palavras)\","
...
def _verificar_promocao_licao(licao: dict) -> bool:
    if licao.get("promovida_para_identidade"): return False
    if licao.get("confirmacoes", 1) >= 2:
        try:
            ...
            if (ultima - primeira) <= timedelta(days=30):
                ...
                warn("LICAO", f"PROMOÇÃO: '{licao.get('gatilho','')[:60]}'")
                return True
        except Exception:
            pass
    return False
...
def registrar_licao(gatilho: str, resposta_errada: str, dados_llm: dict | None = None) -> None:
    ...
    for licao in livro.get("licoes", []):
        if licao.get("status") != "ativo": continue
    ...
    nova = {
        ...
        "tipo": tipo_licao, "confianca": confianca, "origem": "correcao_usuario",
        "confirmacoes": 1,
        "primeira_ocorrencia": datetime.now().strftime("%d/%m/%Y %H:%M"),
        "ultima_ocorrencia": datetime.now().strftime("%d/%m/%Y %H:%M"),
        "promovida_para_identidade": False, "status": "ativo",
    }
...
def buscar_licoes_relevantes(pergunta: str, top_n: int = 3) -> list:
    ...
    def _score(l):
        return (_sim_jaccard(pergunta, l.get("gatilho","")) +
                (10.0 if l.get("tipo") == "anti_padrao" else 0.0) +
                l.get("confianca", 0.95) * 0.3)
...
def formatar_licoes_para_prompt(licoes: list) -> str:
    ...
    for l in licoes:
        ...
        linhas.append(f" [{tag}][{l.get('confirmacoes',1)}x] Quando: '{l.get('gatilho','')}' | "
                      f"NÃO: '{l.get('erro_cometido','')}' | FAÇA: '{l.get('comportamento_correto','')}'")
    return "\n".join(linhas)
...
def _resumo_vazio() -> dict:
    return {"meta": {"versao": "1.0", "gerado_por": "railway_agente", "ultima_geracao": None,
                     "total_entradas_processadas": 0, "ciclos_completos": 0},
            "blocos": {"decisoes": [], "bugs": [], "correcoes": [], "limitacoes": [], "insights": []}}

PROMPT_CLASSIFICAR_LOTE = """Analise estas entradas e classifique cada uma.
Entradas:
{entradas}
Responda APENAS com array JSON:
[{{"indice":0,"bloco":"decisoes|bugs|correcoes|limitacoes|insights|irrelevante","resumo":"max 20 palavras","importancia":0.0,"manter":true}}]
Importância: 0.9+=decisões/correções, 0.7-0.9=bugs/insights, 0.5-0.7=limitações, <0.5=não manter"""

def _classificar_lote(entradas_texto: list) -> list:
    if not entradas_texto: return []
    prompt = PROMPT_CLASSIFICAR_LOTE.format(
        entradas="\n".join([f"{i}: {t[:200]}" for i, t in enumerate(entradas_texto)]))
    try:
        import re
        resposta = chamar_llm(prompt, max_tokens=500)
        match    = re.search(r'\[.*?\]', resposta, re.DOTALL)
        if match: return json.loads(match.group())
    except Exception as e:
        debug("RESUMO", f"Classificação falhou: {e}")
    return []

def executar_sumarizacao():
    log("RESUMO", "Iniciando sumarização...")
    memoria      = carregar_memoria_github() or {}
    aprendizados = memoria.get("aprendizados", [])
    if len(aprendizados) < 5:
        log("RESUMO", "Poucas entradas — pulando")
        return
    resumo = _carregar_livro_local("resumo")
    if not resumo or "blocos" not in resumo: resumo = _resumo_vazio()
    novas_por_bloco   = {b: [] for b in resumo["blocos"]}
    total_processadas = 0
    for i in range(0, len(aprendizados[-50:]), 10):
        lote   = aprendizados[-50:][i:i+10]
        textos = [f"{e.get('pergunta',e.get('tarefa',''))[:100]} → {e.get('resposta',e.get('resultado',''))[:100]}"
                  for e in lote]
        for clf in _classificar_lote(textos):
            if not clf.get("manter", False): continue
            bloco = clf.get("bloco", "irrelevante")
            if bloco not in novas_por_bloco: continue
            idx = clf.get("indice", 0)
            if idx >= len(lote): continue
            novas_por_bloco[bloco].append({
                "id": f"{bloco[:3]}_{uuid.uuid4().hex[:5]}",
                "resumo": clf.get("resumo","")[:100],
                "data": lote[idx].get("data", datetime.now().strftime("%d/%m/%Y")),
    ...
    log("RESUMO", f"Ciclo {resumo['meta']['ciclos_completos']} completo — {total_processadas} entradas")
...
def buscar_resumo_relevante(pergunta: str, top_n: int = 5) -> str:
    ...
    linhas = [f"{emojis.get(b,'•')} [{b.upper()}] {item['resumo']}"
              for b, itens in resumo.get("blocos",{}).items()
              for item in itens if _sim_jaccard(pergunta, item.get("resumo","")) > 0.20]
...
def carregar_aprendizado() -> dict:
    try:
        with open(APRENDIZADO_FILE, "r") as f: return json.load(f)
    except: return {}

def salvar_aprendizado(dados: dict):
    try:
        with open(APRENDIZADO_FILE, "w") as f: json.dump(dados, f, indent=2)
    except: pass
...
def registrar_resultado_api(nome: str, tarefa: str, sucesso: bool, tempo: float):
    ...
    dados[chave]["sucesso" if sucesso else "falha"] += 1
    salvar_aprendizado(dados)

# ── TAREFAS ───────────────────────────────────────────────────────────────────

# FIX: system prompt sem invenção, sem markdown
SYSTEM_PROMPT = (
    "Voce e um agente autonomo rodando em nuvem. "
    "Responda SEMPRE em portugues brasileiro. "
    "Seja direto. Maximo 2 frases curtas. "
    "NUNCA invente dados, datas, registros, servidores ou plataformas. "
    "NUNCA use markdown, asteriscos ou bullet points. "
    "Se nao tiver dado real disponivel: diga apenas sem dados disponiveis."
)
...
def precisa_tavily(pergunta: str) -> bool:
    ...
    if p in CACHE_DECISAO: return CACHE_DECISAO[p]
    if any(x in p for x in PALAVRAS_WEB) and any(x in p for x in ["hoje","agora","atual","tempo real"]):
    ...
    if any(x in p for x in PALAVRAS_BLOQUEIO):
        ...
        return False
    try:
        r = chamar_llm(f"Precisa de dados atuais da internet? Responda APENAS SIM ou NAO.\nTarefa: {pergunta}", max_tokens=3)
...
def contar_registros() -> int:
    try:
        with open("memoria.txt", "r", encoding="utf-8") as f: return len(f.readlines())
    except: return 0
...
def avaliar_tarefa_com_memoria(t: str, mem: dict) -> int:
    ...
    if t in mem.get("tarefas_frequentes", []): peso -= 2
    if t in mem.get("tarefas_evitar", []):     peso -= 3
    if mem.get("busca_recente") and t in ("buscar cotacao dolar", "buscar noticias tech"): peso -= 2
    return max(peso, 0)

def planejar_tarefas(objetivo: str) -> list:
    try:
        import ast
        r = chamar_llm(f"Objetivo: '{objetivo}'. Escolha 3 tarefas desta lista, responda APENAS com lista Python: {tarefas_possiveis}", max_tokens=80)
        tarefas = ast.literal_eval(r)
        if isinstance(tarefas, list): return [t.lower().strip() for t in tarefas]
    except: pass
...
def executar_tarefa_com_retorno(tarefa: str, objetivo: str, registros: int, cpu: float, mem: float) -> str:
    ...
    if tarefa in ("buscar cotacao dolar", "buscar noticias tech"):
        ...
        resultado = buscar_tavily(queries[tarefa])
    elif tarefa == "mostrar hora":
        resultado = agora
    elif tarefa == "monitorar sistema":
        resultado = f"CPU: {cpu}% | Mem: {mem}%"
    elif tarefa == "verificar sistema":
        resultado = chamar_llm(f"CPU {cpu}%, Mem {mem}%, Hora {agora}. Sistema ok?", max_tokens=60, system=SYSTEM_PROMPT)
    elif tarefa == "registrar atividade":
        resultado = f"Atividade registrada as {agora}"
    elif tarefa == "analisar memoria":
        resultado = chamar_llm(f"{registros} registros. Objetivo: {objetivo}. O que fazer?", max_tokens=60, system=SYSTEM_PROMPT)
    elif tarefa == "limpar memoria":
        try:
            open("memoria.txt", "w", encoding="utf-8").close()
            resultado = "Memoria limpa"
        except Exception as e:
            resultado = f"Erro: {e}"
    elif tarefa == "gerar relatorio":
        resultado = chamar_llm(f"Resumo: {registros} registros, CPU {cpu}%, Mem {mem}%.", max_tokens=60, system=SYSTEM_PROMPT)
    else:
        resultado = chamar_llm(f"Execute: '{tarefa}'. CPU {cpu}%, Mem {mem}%.", max_tokens=80, system=SYSTEM_PROMPT)
    ...
    return resultado or "Tarefa executada."
...
def loop_agente():
    ...
    while not _encerrando["v"]:
        try:
            ...
            if not tarefas: tarefas.extend(planejar_tarefas(objetivo))
            if cpu > 70: tarefas.append("analisar memoria")
            if mem > 70: tarefas.append("limpar memoria")
            if contador % 30 == 0: tarefas.append("buscar cotacao dolar")
            ...
            if _deve_sumarizar():
                log("RESUMO", "Gatilho — sumarizando em background")
                threading.Thread(target=executar_sumarizacao, daemon=True).start()
            time.sleep(60)
        except Exception as e:
            erro("LOOP", str(e))
